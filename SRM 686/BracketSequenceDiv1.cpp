...
void initok(string s) {
    int l = SZ(s);
    for (int i=0; i<l; i++) {
        for (int j=i+1; j<l; j++) ok[i][j] = check(s, i, j);
    }
}

llong dfs(int l, int r) {
    if (l > r) return 1;
    if (f[l][r] != -1) return f[l][r];
    if (l == r) return f[l][r]=1; //empty

    llong ans = 0;
    ans += dfs(l+1, r); // delete s[l]
    for (int i=l+1; i<=r; i++) if (match(ss, l, i)) { // not delete s[l]
        llong t1 = dfs(l+1, i-1);
        llong t2 = dfs(i+1, r);
        ans += t1 * t2;
    }
    f[l][r] = ans;
    //dbg(l); dbg(r); dbg(f[l][r]); dbg(ok[l][r]);
    return f[l][r];
}
